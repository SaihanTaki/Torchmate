...
class WandbModelCheckpoint(Callback):
    ...
    This callback saves model checkpoints in wandb cloud at specified intervals and under certain conditions such as best
    ...
    Parameters:
        ...
        save_state_dict_only (bool, optional): Whether to save only the model's state dictionary instead of the whole model. Defaults to ``True``.
    ...
    **Example Usage:**
    ...
    def on_epoch_end(self,trainer):
        ...
        if self.save_best_only and condition:
            ...
            checkpoint_name = f'model_best.pt'
            checkpoint_path = os.path.join(checkpoint_dir,checkpoint_name)
            torch.save(model, checkpoint_path)
            wandb.save(checkpoint_path, base_path=self.checkpoint_dir, policy="live")
        ...
        if self.save_last and self.epoch_count==trainer.num_epochs:
            filepath = os.path.join(checkpoint_dir,f"model_last.pt")
