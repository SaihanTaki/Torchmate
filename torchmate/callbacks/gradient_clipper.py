    ...
    **Example Usage:**
    ...
    def __init__(
        ...
        if self.method == "clip_by_value" and self.clip_value is None:
            raise ValueError(
                f"clip_value must be provided when method is '{method}'"
            )

        if self.method == "clip_by_norm" and self.max_norm is None:
            raise ValueError(
                f"max_norm must be provided when method is '{method}'"
            )
    ...
    def on_backward_end(self, trainer) -> None:
        ...
        if self.method == "clip_by_norm":
            torch.nn.utils.clip_grad_norm_(
                trainer.model.parameters(), max_norm=self.max_norm
            )
    
        elif self.method == "clip_by_value":
            torch.nn.utils.clip_grad_value_(
                trainer.model.parameters(), clip_value=self.clip_value
            )